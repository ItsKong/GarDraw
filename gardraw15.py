...
create_room_button = pygame.Rect(
    (WIDTH - button_width) // 2, 260, button_width, button_height
)
join_room_button = pygame.Rect(
    (WIDTH - button_width) // 2, 300, button_width, button_height
)
drawer_demo_button = pygame.Rect(
    (WIDTH - button_width) // 2, 340, button_width, button_height
)
guesser_demo_button = pygame.Rect(
    (WIDTH - button_width) // 2, 380, button_width, button_height
)
...
input_box = pygame.Rect(
    (WIDTH - input_box_width) // 2,
    create_room_button.y - 70,
    input_box_width,
    input_box_height,
)
...
menu_background = pygame.Surface(
    (button_width + 20, button_height * 5 + 150), pygame.SRCALPHA
)
...
for i, color_data in enumerate(color_palette):
    ...
    color_buttons.append(
        {
            "rect": pygame.Rect(
                x_pos, color_palette_y, color_button_size, color_button_size
            ),
...
for i, size in enumerate(brush_sizes):
    ...
    brush_size_buttons.append(
        {
            "rect": pygame.Rect(
                x_pos, HEIGHT - toolbar_height + 5, brush_button_size, brush_button_size
            ),
            "size": size,
        }
    )
...
def update_word_hint():
    ...
    if (
        current_time - last_reveal_time > word_reveal_interval
        and revealed_letters < len(selected_word)
    ):
...
def check_guess(guess):
    ...
    if guess == correct_word:
        ...
        chat_messages.append(
            f"System: {user_data['name']} guessed correctly! +{score} points!"
        )
...
def draw_scoreboard():
    ...
    round_text = font.render(
        f"Round: {game_session['round']}/{game_session['max_rounds']}", True, WHITE
    )
    ...
    if game_state in [DRAWING, GUESSING]:
        remaining = (
            max(0, game_session["round_end_time"] - pygame.time.get_ticks()) // 1000
        )
    ...
    if game_session["current_drawer"]:
        drawer_text = font.render(
            f"Drawer: {game_session['current_drawer']}", True, WHITE
        )
...
def draw_chat():
    ...
    if (
        ...
        and user_data["name"] not in game_session["correct_guessers"]
    ):
...
def start_new_round():
    ...
    if not game_session["players"]:
        ...
        if game_session["current_drawer"] is None:
            ...
            try:
                current_index = game_session["players"].index(
                    game_session["current_drawer"]
                )
    ...
    # Set user role
    user_data["role"] = (
        "drawer" if user_data["name"] == game_session["current_drawer"] else "guesser"
    )
    ...
    game_session["round_end_time"] = game_session["round_start_time"] + (
        game_session["round_duration"] * 1000
    )
...
def end_round():
    ...
    chat_messages.append(
        f"System: {game_session['current_drawer']} earned {drawer_score} points as drawer!"
    )
...
def end_game_session():
    ...
    if scores:
        ...
        chat_messages.append(
            f"System: Game over! Winner is {winner[0]} with {winner[1]} points!"
        )
...
def all_players_guessed():
    ...
    remaining_guessers = (
        all_players
        - {game_session["current_drawer"]}
        - set(game_session["correct_guessers"])
    )
        ...
        if event.type == pygame.QUIT:
            ...
            # get keyboard input
                ...
                if event.key == pygame.K_RETURN:
                    ...
                    user_data["name"] = user_text
                    save_user_data()
                    status_message = f"Name saved: {user_text}"
                    status_timer = 60
                    active = False
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1:
                    ...
                    if back_button.collidepoint(event.pos):
                        ...
                        chat_input_box.collidepoint(event.pos)
                        and user_data["role"] == "guesser"
                        and user_data["name"] not in game_session["correct_guessers"]
                    ):
                        chat_active = True
                    else:
                        ...
                        if event.pos[
                            1
                        ] < HEIGHT - toolbar_height and not chat_input_box.collidepoint(
                            event.pos
                        ):
                ...
                if drawing and event.pos[1] < HEIGHT - toolbar_height:
                    ...
                    if last_pos:
                        active_color = (
                            WHITE if current_mode == ERASE_MODE else current_color
                        )
                        active_brushsize = (
                            brush_size + 20
                            if current_mode == ERASE_MODE
    ...
    if game_state == MENU:
        ...
        if active:
            ...
            text_surface = font.render(
                placeholder_text if user_text == "" else user_text,
                True,
                GRAY if user_text == "" else BLACK,
            )
        ...
        screen.blit(
            guesser_text, (guesser_demo_button.x + 65, guesser_demo_button.y + 10)
        )
        ...
        if status_timer > 0:
            ...
            status_rect = status_text.get_rect(
                center=(WIDTH // 2, quit_button.bottom + 30)
            )
            pygame.draw.rect(
                screen, BLACK, status_rect.inflate(20, 10), border_radius=5
            )
        ...
        if user_data["role"] == "drawer":
            ...
            # Draw tool buttons
            pygame.draw.rect(
                screen, GREEN if current_mode == PEN_MODE else GRAY, pen_button
            )
            ...
            pygame.draw.rect(
                screen, GREEN if current_mode == FILL_MODE else GRAY, bucket_button
            )
            ...
            pygame.draw.rect(
                screen, GREEN if current_mode == ERASE_MODE else GRAY, eraser_button
            )
            ...
            for i, btn in enumerate(brush_size_buttons):
                ...
                if i == brush_size_index:
                    pygame.draw.rect(
                        screen, WHITE, btn["rect"].inflate(4, 4), 2, border_radius=7
                    )

                dot_color = current_color if current_mode != ERASE_MODE else WHITE
                dot_size = (
                    btn["size"]
                    if btn["size"] < brush_button_size - 10
                    else brush_button_size - 10
                )
            ...
            if current_mode == ERASE_MODE:
                ...
                mode_text = font.render(
                    f"Mode: Paint Bucket | Color: {color_palette[current_color_index]['name']}",
                    True,
                    WHITE,
                )
            else:
                mode_text = font.render(
                    f"Mode: Pen | Color: {color_palette[current_color_index]['name']}",
                    True,
                    WHITE,
                )
        ...
        if status_timer > 0:
            ...
            status_text = font.render(status_message, True, WHITE)
            status_rect = status_text.get_rect(
                center=(WIDTH // 2, HEIGHT - toolbar_height - 20)
            )
            pygame.draw.rect(
                screen, BLACK, status_rect.inflate(20, 10), border_radius=5
            )
